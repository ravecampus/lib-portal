        ...
        <div class="row card">
            <div class="col-md-12 card-body">
                ...
                <data-table class="mt-5" :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy">
                    <tbody v-for="(list, idx) in galleries" :key="idx">
                        <tr class="tr-shadow">
                            <td>
                ...
                <div class="col-md-12">
                    <div class="pull-right">
                        <pagination :pagination="pagination"
                            @prev="listGallery(pagination.prevPageUrl)"
                            @next="listGallery(pagination.nextPageUrl)"
                            v-show="noData(galleries)">
                        </pagination>
                    </div>
            ...
            <div class="modal fade gallery" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    ...
                    <div class="modal-header">
                        ...
                        <div class="modal-footer text-center">
                            <button type="button" @click="saveGallery()" class="btn btn-success">{{ btncap }}</button>
            ...
            <div class="modal fade delete-gallery">
                <div class="modal-dialog modal-sm" role="document">
                    ...
                    <div class="modal-header">
                        ...
                        <div class="modal-footer text-center">
                            <div class="btn-group">
                                <button type="button" @click="confirmDeleteGallery(post)" class="btn btn-danger btn-sm">Yes</button>
    ...
    methods:{
        ...
        saveGallery(){
            if(this.post.id > 0){
                ...
                this.btncap = "Saving...";
                this.$axios.put("api/gallery/"+this.post.id, this.post).then(res=>{
                    this.btncap = "Save";
                    this.$emit('show',{'message':'Gallery has been modified!', 'status':4});
                    this.post ={};
                    this.errors = [];
                    $('.gallery').modal('hide');
                    this.listGallery();
                ...
                this.$axios.get("sactum/cookie-csrf").then(response=>{
                    this.btncap = "Saving...";
                    this.$axios.post("api/gallery", this.post).then(res=>{
                        this.$emit('show',{'message':'Gallery has been saved!', 'status':4});
                        this.btncap = "Save";
                        this.post ={};
                        this.errors = [];
                        $('.gallery').modal('hide');
        ...
        confirmDeleteGallery(data){
            this.$axios.get("sactum/cookie-csrf").then(response=>{
                this.$axios.delete("api/gallery/"+data.id).then(res=>{
                    this.$emit('show',{'message':'Gallery has been deleted!', 'status':4});
        ...
        uploadLogo(){
            ...
            this.$axios.get('sanctum/csrf-cookie').then(response=>{
                this.$axios.post('api/gallery-upload',this.form, config).then(res=>{
                    this.showbar = false;
